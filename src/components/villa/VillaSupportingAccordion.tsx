...
import { Clock3, HelpCircle, MapPin } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
type TravelerFaq = { question: string; answer: string };
type StayRule = { title: string; detail: string; icon: LucideIcon };
type NearbySpot = { label: string; distance: string; icon: LucideIcon };
...
export function VillaSupportingAccordion({ faqs, rules, nearby }: VillaSupportingAccordionProps) {
  return (
    <section
    ...
    >
      ...
      {/* Single accordion with multiple — all items can be open simultaneously */}
      <Accordion type="multiple" className="w-full divide-y divide-slate-100">

        {/* ── 1. Stay Rules ── */}
        {rules.length > 0 && (
          <AccordionItem value="rules" className="border-0">
            <AccordionTrigger className="flex items-center gap-3 px-5 py-4 text-left hover:bg-slate-50/70 hover:no-underline sm:px-6 [&>svg]:ml-auto [&>svg]:flex-shrink-0">
              <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-[#3A4A1F]/8">
                <Clock3 className="h-4 w-4 text-[#3A4A1F]" aria-hidden="true" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Kebijakan</p>
                <p className="text-[15px] font-bold text-slate-900">Aturan Menginap</p>
              </div>
            </AccordionTrigger>
            <AccordionContent className="pb-0">
              <div className="grid gap-3 px-5 pb-5 sm:grid-cols-2 sm:px-6 sm:pb-6">
                {rules.map((rule) => {
                  ...
                  return (
                    <div key={rule.title} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                      <div className="mb-2 flex items-center gap-2">
                        <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-white shadow-sm">
                          <Icon className="h-3.5 w-3.5 text-[#3A4A1F]" aria-hidden="true" />
                        </div>
                        <p className="text-sm font-bold text-slate-900">{rule.title}</p>
                      </div>
                      <p className="text-[13px] leading-6 text-slate-500">{rule.detail}</p>
                    </div>
                  );
                })}
              </div>
        ...
        {faqs.length > 0 && (
          <AccordionItem value="faq" className="border-0">
            <AccordionTrigger className="flex items-center gap-3 px-5 py-4 text-left hover:bg-slate-50/70 hover:no-underline sm:px-6 [&>svg]:ml-auto [&>svg]:flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Pertanyaan Umum</p>
                <p className="text-[15px] font-bold text-slate-900">FAQ Tamu</p>
              </div>
            </AccordionTrigger>
            <AccordionContent className="pb-0">
                ...
                {faqs.map((faq, index) => (
                  <div key={faq.question} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                    <div className="mb-2 flex items-start gap-2.5">
                      <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-[#3A4A1F]/8 text-[10px] font-bold text-[#3A4A1F]">
                        {index + 1}
                      </span>
                      <p className="text-sm font-bold text-slate-900">{faq.question}</p>
                    </div>
                    <p className="pl-7 text-[13px] leading-6 text-slate-500">{faq.answer}</p>
                  </div>
                ))}
              </div>
            </AccordionContent>
        ...
        {nearby.length > 0 && (
          <AccordionItem value="nearby" className="border-0">
            <AccordionTrigger className="flex items-center gap-3 px-5 py-4 text-left hover:bg-slate-50/70 hover:no-underline sm:px-6 [&>svg]:ml-auto [&>svg]:flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Lingkungan</p>
                <p className="text-[15px] font-bold text-slate-900">Lokasi Sekitar</p>
              </div>
            </AccordionTrigger>
            <AccordionContent className="pb-0">
                  ...
                  return (
                    ...
                    >
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <p className="truncate text-sm font-semibold text-slate-800">{spot.label}</p>
                      </div>
